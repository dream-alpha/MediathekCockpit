...
class EventView(Screen):
	...
	def __init__(self, session, curr):
		...
		self["key_red"] = Button(_("Abort"))
		self["key_green"] = Button()
		self["key_yellow"] = Button()
		self["key_blue"] = Button()
		epg_description = curr[LIST_EVENT_NAME]
		if curr[LIST_SHORT_DESCRIPTION] and curr[LIST_SHORT_DESCRIPTION] not in curr[LIST_EVENT_NAME]:
		...
		self["duration"] = Label(str(curr[LIST_DURATION]))
