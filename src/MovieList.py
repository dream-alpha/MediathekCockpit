...
class MovieList:
    ...
    def download(self, postdata, channel=None):
        ...
        if result.text:
            ...
            result_count = min(
                postdata["offset"] + query_info["resultCount"], query_info["totalResults"])
            ...
            if query_info["totalResults"] != result_count:
                alist.append(self.getNextRow())
    ...
    def getMovieRow(self, x):
        ...
        series_episode = self.getSeriesEpisode(title)
    ...
    def getSeriesEpisode(self, text):
        """
